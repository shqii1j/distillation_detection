...
class StableDiffusionPipeline(
    ...
    def __call__(
        ...
        clip_skip: Optional[int] = None,
        saved_steps: Optional[List[float]] = None,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.saved_steps is not None and i in self.saved_steps:
                    stat_dict[t.item()]['mean'].update(latents.mean([0, 2, 3]))
                    stat_dict[t.item()]['var'].update(latents.var([0, 2, 3], unbiased=False))
        ...
        if not return_dict:
            if self.saved_steps is not None:
                return ((image, has_nsfw_concept), stat_dict)
            return (image, has_nsfw_concept)
        
        if self.saved_steps is not None:
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), stat_dict
